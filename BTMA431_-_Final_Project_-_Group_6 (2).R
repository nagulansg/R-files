...
# NOTE: We are using Spotify's web API to access fresh data each time this code is run, and the data on Spotify
# is bound to change day-by-day. Due to this, the exact results mentioned in our comments and report may vary
# from the results found when running this code at a later date. All code comments and data in our report are aligned
# with our findings on December 8, 2021.
...
hist(Residuals,col='#2AC831',main = "Histogram of Danceability Model Residuals",xlim=c(-0.3,0.3)) # The distribution is roughly centered at zero
...
# The AIC approach selected Valence, Acousticness, Key, Energy and Instrumentalness as predictors 
...
# Although the AIC approach also selected Energy and Instrumentalness, we decided not to include these predictors in order to reduce complexity and to align with the findings of our regression model as well as the BIC approach.
...
# Creating a regression model to test the impact of other variables on popularity
popularity.data <- Spotify.Data[, c("Popularity", "Danceability", "Valence", "Acousticness", "Key", "Instrumentalness", "Liveliness", "Loudness", "Tempo", "Mode", "Energy", "Danceability", "Duration")]
popularity_model <- lm(Popularity ~ Danceability + Valence + Acousticness + Key + Instrumentalness + Liveliness + Loudness + Tempo + Mode + Energy + Duration, data = Spotify.Data)
summary(popularity_model) # As per the p-values, we see that Liveliness and Key are the most significant predictors

# In order to use the p-values to determine which variables to include in our model, we need to check the Regression assumptions: 
# a) Errors have mean zero & errors look Normally Distributed
hist(popularity_model$residuals,col='#2AC831',breaks=30,main = "Histogram of Popularity Model Residuals")  # The distribution is roughly centered at zero
autoplot(popularity_model,which =2, ncol = 1) # We can see that the bulk of the points fall on the dashed line, meaning the distribution of the residuals matches the Normal distribution (dashed lines).

# b) Errors have common variance & should be uncorrelated with each predictor 
par(mfrow=c(2,2))
par(mar=c(1,1,1,1))
plot(popularity_model,3) # Based on this plot, we conclude homogeneity of variance since our plot shows that the residuals are spread equally along the ranges of predictors.

# Checking the multicollinearity between variables 
vif(popularity_model) # None of the variables have a VIF greater than 5 meaning that they are not correlated/linearly dependent with the others

# # As another check to ensure we are including the right predictors in our model, we use the AIC model selection technique 
null.model1 <- lm (Popularity ~ 1, data = popularity.data)
full.model1 <- lm(Popularity ~ ., data = popularity.data)
step.both.AIC1 <- step(null.model1, scope = list(lower=null.model1, upper=full.model1), direction="both")
summary(step.both.AIC1) 
# The AIC Test indicates that the best predictors are Liveliness, Key, and Duration. 
# When analyzing the p-values, duration was not a significant predictor, however we will be including it in our model as the AIC approach accounts for false negatives. 

average.duration <- mean(Spotify.Data$Duration)
average.duration # Since all the artists in Spotify.Data have a popularity of 91 and over, we know that this dataset
# only includes the most popular artists. As such, we can see that the average duration among the most popular
# artists is 224299.9 ms, which translates to 3.73 minutes. The duration for the most popular songs is in the 
# 3 to 4 minute mark.

# Only including the significant predictors we get the following for our model: 
popularity_model_sig_predictors <- lm(Popularity ~ Liveliness + Key + Duration, data=Spotify.Data)
summary(popularity_model_sig_predictors)
